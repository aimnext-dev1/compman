...
COMPOSE_FILE=$(find "$DEPLOY_HOME" -maxdepth 1 -name 'docker-compose.yml' -o -name 'docker-compose.yaml' -o -name 'compose.yml' -o -name 'compose.yaml' | head -n 1)

# 파일이 존재하는지 확인
if [ -z "$COMPOSE_FILE" ]; then
  echo "$DEPLOY_HOME 경로에 스택 명세 파일이 존재하지 않습니다."
