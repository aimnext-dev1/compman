...
CONFIG_FILE_PATH_FROM_HOST="$CONFIG_HOME/filebeat.yml"

#키 값 변경
console_out "값을 설정합니다.."
sed -i "s|var.access_key_id: '.*'|var.access_key_id: '$VALUE_ACCESS_KEY'|" "$CONFIG_FILE_PATH_FROM_HOST"
sed -i "s|var.secret_access_key: '.*'|var.secret_access_key: '$VALUE_SECRET_KEY'|" "$CONFIG_FILE_PATH_FROM_HOST"

CONTAINER_NAME="project-data-es-filebeat-filebeat-1"
CONTAINER_KEY=$(docker ps -a | grep $CONTAINER_NAME | xargs | awk '{print $1}')

# 적용할 설정파일 경로
CONFIG_FILE_PATH_FROM_CONTAINER="/usr/share/filebeat/filebeat.yml"

console_out "수정한 설정파일을 컨테이너로 전달합니다."
docker cp "$CONFIG_FILE_PATH_FROM_HOST" "$CONTAINER_KEY":$CONFIG_FILE_PATH_FROM_CONTAINER
