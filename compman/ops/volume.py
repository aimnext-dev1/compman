...
def backup(
    ...
    if not no_stop:
        click.echo("Stopping stack for consistent backup...")
    ...
    if not no_stop:
        click.echo("Starting stack again...")
    ...
    click.echo(f"Volume backup done: {tarball}")
...
def restore(
    ...
    if not map_path.is_file():
        click.echo(f"volume-map.json not found in backup.", err=True)
    ...
    if not no_stop:
        click.echo("Stopping stack for consistent restore...")
        runtime.run_compose(["stop"], project=config.name, capture=False)
    ...
    for container, vol_info in mapping.items():
        ...
        if not src.is_dir():
            click.echo(f"Warning: data dir '{src}' not found, skipping {container}.")
            continue
        click.echo(f"Restoring {container}:{dest} ...")
        runtime.run_cli(
    ...
    shutil.rmtree(restore_dir)
    click.echo("Volume restore done.")
...
def push(runtime: ContainerRuntime, config: Config) -> None:
    ...
    for container, vol_info in mapping.items():
        ...
        if not src.is_dir():
            click.echo(f"Warning: '{src}' not found, skipping {container}.")
            continue
        click.echo(f"Pushing to {container}:{dest} ...")
        runtime.run_cli(
            ["cp", f"{str(src)}/.", f"{container}:{dest}"],
            capture=False,
        )
        _fix_permissions(runtime, container, dest)
    click.echo("Volume push done.")
...
def _fix_permissions(
    ...
    click.echo(f"Fixing permissions on {container}:{dest} ...")
    r = runtime.run_cli(
        [
            "exec",
            container,
...
def _list_backups(config: Config, kind: str) -> None:
    ...
    click.echo(f"Available {kind} backups:")
    for f in sorted(config.backup_dir.glob(f"{pattern}*.tar.gz")):
        name = f.name
        ts = name.replace(pattern, "").replace(".tar.gz", "")
        click.echo(f"  {ts}")  # ponytail: color omitted, click handles terminal
